...
GoはGoogleで開発されたプログラミング言語です。Goはマシン語にコンパイルされてから実行されるため、PythonやRubyなどのインタプリタ言語と比較して高速です。また、ガベージコレクションや型推論等の仕組みを持っているため、C言語よりも手軽にプログラミングすることができます。Go自体は@<strong>{Tour of Go}(https://go-tour-jp.appspot.com/welcome/1)で学ぶことができますので、Goに触れたことがない方はまずそちらを一通り触ってみると良いかと思います。
...
printする場合は、フォーマット指定子に@<tt>{%x}や@<tt>{%q}を用いるか、"encoding/hex"パッケージの dump 関数を用いると便利です。
...
Goでは@<strong>{bytes}パッケージにbyte列を操作する関数が提供されています。
